...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Local Difix3D-style Nerfstudio trainer.")

    parser.add_argument("--data", type=Path, default=None, help="Path to dataset dir/json or DL3DV scene dir.")
    parser.add_argument("--scene_id", type=str, default=None, help="DL3DV scene id when using --dl3dv_root.")
    parser.add_argument(
        "--dl3dv_root",
        type=Path,
    ...
    parser.add_argument("--method_name", type=str, default="difix3d")
    parser.add_argument(
        "--timestamp",
        type=str,
        default="{timestamp}",
    ...
    parser.add_argument("--steps_per_eval_batch", type=int, default=0)
    parser.add_argument("--steps_per_eval_image", type=int, default=0)
    parser.add_argument("--steps_per_eval_all_images", type=int, default=0)
    parser.add_argument("--steps_per_save", type=int, default=2000)
    parser.add_argument("--steps_per_fix", type=int, default=2000)
    ...
    parser.add_argument("--center_method", choices=["poses", "focus", "none"], default="none")
    parser.add_argument("--auto_scale_poses", action=argparse.BooleanOptionalAction, default=False)

    parser.add_argument("--train_num_rays_per_batch", type=int, default=16384)
    ...
    parser.add_argument("--appearance_embed_dim", type=int, default=0)
    parser.add_argument("--camera_optimizer_mode", choices=["off", "SO3xR3", "SE3"], default="off")
    parser.add_argument("--implementation", choices=["tcnn", "torch"], default="tcnn")
    parser.add_argument("--cache_num_workers", type=int, default=0)
    parser.add_argument("--patch_size", type=int, default=32)
    parser.add_argument("--disable_camera_optimizer", action="store_true")
    parser.add_argument("--freeze_appearance_embedding", action="store_true")
...
def _resolve_data_path(
    ...
    candidate = data.expanduser().resolve() if data is not None else (dl3dv_root.expanduser().resolve() / scene_id).resolve()
    ...
    if (candidate / "transforms.json").exists():
        inferred = scene_id if scene_id else candidate.parent.name if candidate.name in {"nerfstudio", "gaussian_splat"} else candidate.name
        return candidate, inferred

    for src in _candidate_sources(scene_source):
        subdir = candidate / src
        if (subdir / "transforms.json").exists():
            inferred = scene_id if scene_id else candidate.name
            return subdir, inferred

    looked_for = ", ".join(_candidate_sources(scene_source))
    raise FileNotFoundError(f"Could not find transforms.json in {candidate} or in its [{looked_for}] subfolders.")
...
def _ensure_nerfstudio_import_path(nerfstudio_path: Optional[Path]) -> None:
    ...
    if not (path / "nerfstudio").is_dir():
        raise FileNotFoundError(f"--nerfstudio_path must contain a `nerfstudio` package directory. Got: {path}")
    path_str = str(path)
...
def _load_symbols() -> Dict[str, Any]:
    return {
        ...
        "ExponentialDecaySchedulerConfig": _import_symbol(
            "nerfstudio.engine.schedulers",
            "ExponentialDecaySchedulerConfig",
        ),
        "Difix3DTrainerConfig": _import_symbol(
            "scripts.nerfstudio_difix3d.difix3d_trainer",
            "Difix3DTrainerConfig",
        ),
        "Difix3DPipelineConfig": _import_symbol(
            "scripts.nerfstudio_difix3d.difix3d_pipeline",
            "Difix3DPipelineConfig",
        ),
        "Difix3DDataManagerConfig": _import_symbol(
            "scripts.nerfstudio_difix3d.difix3d_datamanager",
            "Difix3DDataManagerConfig",
        ),
        "Difix3DModelConfig": _import_symbol(
            "scripts.nerfstudio_difix3d.difix3d",
            "Difix3DModelConfig",
        ),
    }
...
def _build_config(args: argparse.Namespace, data_path: Path, experiment_name: str, ns: Dict[str, Any]) -> Any:
    ...
    dataparser_params = inspect.signature(ns["NerfstudioDataParserConfig"]).parameters
    if "auto_scale_poses" in dataparser_params:
        dataparser_kwargs["auto_scale_poses"] = args.auto_scale_poses
    dataparser_cfg = ns["NerfstudioDataParserConfig"](**dataparser_kwargs)
    datamanager_cfg = ns["Difix3DDataManagerConfig"](
    ...
    model_cfg = ns["Difix3DModelConfig"](
        eval_num_rays_per_chunk=args.eval_num_rays_per_chunk,
        average_init_density=args.average_init_density,
        appearance_embed_dim=args.appearance_embed_dim,
        camera_optimizer=ns["CameraOptimizerConfig"](mode=args.camera_optimizer_mode),
        implementation=args.implementation,
        disable_camera_optimizer=args.disable_camera_optimizer,
        freeze_appearance_embedding=args.freeze_appearance_embedding,
    )
    pipeline_cfg = ns["Difix3DPipelineConfig"](
        datamanager=datamanager_cfg,
        model=model_cfg,
        steps_per_fix=args.steps_per_fix,
        ...
        "optimizers": {
            "proposal_networks": {
                ...
                "scheduler": ns["ExponentialDecaySchedulerConfig"](
                    lr_final=args.proposal_lr_final,
                    max_steps=args.proposal_lr_decay_steps,
                ),
            },
            "fields": {
                "optimizer": ns["AdamOptimizerConfig"](lr=args.fields_lr, eps=1e-15),
                "scheduler": ns["ExponentialDecaySchedulerConfig"](
                    lr_final=args.fields_lr_final,
                    max_steps=args.fields_lr_decay_steps,
                ),
            },
            "camera_opt": {
                "optimizer": ns["AdamOptimizerConfig"](lr=args.camera_opt_lr, eps=1e-15),
                "scheduler": ns["ExponentialDecaySchedulerConfig"](
                    lr_final=args.camera_opt_lr_final,
                    max_steps=args.camera_opt_lr_decay_steps,
                ),
            },
        },
    ...
    trainer_params = inspect.signature(ns["Difix3DTrainerConfig"]).parameters
    if "steps_per_eval_image" in trainer_params:
        trainer_kwargs["steps_per_eval_image"] = args.steps_per_eval_image
    if "steps_per_eval_all_images" in trainer_params:
        trainer_kwargs["steps_per_eval_all_images"] = args.steps_per_eval_all_images
    cfg = ns["Difix3DTrainerConfig"](**trainer_kwargs)
    if args.load_checkpoint is not None:
        if not hasattr(cfg, "load_checkpoint"):
            raise AttributeError("This Nerfstudio TrainerConfig does not expose `load_checkpoint`.")
...
def main() -> None:
    ...
    try:
        ...
        raise RuntimeError(
            "Failed to import Nerfstudio or the local Difix3D plugin. Install nerfstudio>=0.3.0 "
            "or pass --nerfstudio_path to a directory containing the `nerfstudio` package."
        ) from exc
    ...
    if cfg.machine.num_devices != 1 or cfg.machine.num_machines != 1:
        raise ValueError(
            "This launcher currently supports single-process training only "
