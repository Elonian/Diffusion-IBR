...
class EulerDiscreteScheduler(SchedulerMixin, ConfigMixin):
    ...
    def step(
        ...
        warp_latent: Optional[torch.Tensor] = None,
        warp_mask: Optional[torch.Tensor] = None,
        ...
        if self.config.prediction_type == "original_sample" or self.config.prediction_type == "sample":
            ...
            pred_original_sample = model_output * (-sigma / (sigma**2 + 1) ** 0.5) + (sample / (sigma**2 + 1)) #[B, frames, D=4, H, W]
        else:
            raise ValueError(
                f"prediction_type given as {self.config.prediction_type} must be one of `epsilon`, or `v_prediction`"
            )
        
        # ! guide pred_original_sample
        if (guide and prior_latents is not None) or (warp and warp_latent is not None):
            pred_original_sample = self.fuse_latents(
                pred_original_sample, guide, prior_latents, mask, warp, warp_latent, warp_mask, latent_height, latent_width
            )
    ...
    def fuse_latents(
        ...
        if prior_latents is not None:
            prior_latents = prior_latents.to(dtype=x0.dtype)
        if warp_latent is not None:
            warp_latent = warp_latent.to(dtype=x0.dtype)
        
        if guide and prior_latents is not None:
            mask = mask.unsqueeze(0)
            mask = resize(mask, (latent_height, latent_width))[None, ...]
            # mask *= 0.2

        if warp and warp_latent is not None:
            warp_mask = warp_mask.unsqueeze(0)
            warp_mask = resize(warp_mask, (latent_height, latent_width))[None, ...]
            warp_latent = warp_latent * warp_mask + x0 * (1-warp_mask)
            # warp_mask = (resize(warp_mask, (latent_height, latent_width)) > 0.5).view(1, -1, 1)
            # warp_latent[~warp_mask[..., 0]] = x0[~warp_mask[..., 0]]

        if guide and warp:
            x0 = prior_latents*mask + (1-mask)*(warp_latent * 0.8 + x0 * 0.2)
            # x0 = prior_latents*mask + (1-mask) * ibr_latent
        elif guide and not warp:
            x0 = prior_latents*mask + (1-mask)*x0
        else:
            x0 = warp_latent * 0.8 + x0 * 0.2
        # pred_original_sample = prior_latents*mask + pred_original_sample*(1-mask)
