...
class CustomSDXLFixer:
    ...
    def __init__(
        ...
        try:
            ...
            raise RuntimeError(
                "CustomSDXLFixer requires the embedded FreeFix SDXL pipeline under "
                f"{FREEFIX_IMPL_ROOT}."
            ) from exc
    ...
    def __call__(
        ...
        if mask is not None:
            ...
            mask_tensor = torch.zeros((1, height, width), dtype=torch.float32, device=self.device)
            mask_scheduler = [max(1, int(num_inference_steps * strength))]
            guide_until = 0
